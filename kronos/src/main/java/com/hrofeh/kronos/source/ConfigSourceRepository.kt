	...
	fun <T : Any> getSource(sourceDefinition: SourceDefinition<T>): ConfigSource {
		...
		if (sourceDefinition is SourceDefinition.IdentifiableClass<T>) {
			...
			configSourceFactory?.let { factory ->
				com.hrofeh.kronos.Kronos.logger?.v("Found factory for id = ${sourceDefinition.id}, generating source")
		...
		com.hrofeh.kronos.Kronos.logger?.v("Unable to find source for id = $sourceDefinition, returning stub source")
